...
@section('title', 'Input Masakan Dapur Harian - ' . ($activeBranch->name ?? 'Cabang'))

@section('content')
<div class="space-y-6">
    <!-- Unified Header Card (Matching Edit Page Layout) -->
    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                ...
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2 flex-wrap">
                    <span>Input Masakan Dapur {{ $activeBranch->name }}</span>
                    @if($isBackday)
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>
                            <span>Mode Susulan (H-{{ $daysDiff }})</span>
                        </span>
                    @else
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-800 border border-emerald-200 flex items-center gap-1">
                ...
                <p class="text-xs text-gray-500 mt-1">
                    Tanggal Laporan Ini: <strong class="text-gray-800">{{ \Carbon\Carbon::parse($dateString)->translatedFormat('l, d F Y') }}</strong>
                </p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2.5 flex-wrap">
                <div class="flex items-center gap-1.5 flex-wrap bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                    ...
                    <button 
                        ...
                        class="px-2.5 py-1.5 {{ $dateString === $yesterdayString ? 'bg-amber-600 text-white' : 'bg-white hover:bg-slate-200 text-slate-700' }} text-[11px] font-bold rounded-lg border border-slate-200 shadow-2xs transition inline-flex items-center gap-1 cursor-pointer"
                    >
                        <svg class="w-3.5 h-3.5 {{ $dateString === $yesterdayString ? 'text-white' : 'text-amber-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <span>Kemarin (H-1)</span>
                    </button>
                    <button 
                        type="button" 
                        onclick="setQuickDate('{{ $twoDaysAgoString }}')" 
                        class="px-2.5 py-1.5 {{ $dateString === $twoDaysAgoString ? 'bg-amber-600 text-white' : 'bg-white hover:bg-slate-200 text-slate-700' }} text-[11px] font-bold rounded-lg border border-slate-200 shadow-2xs transition inline-flex items-center gap-1 cursor-pointer"
                    >
                        <svg class="w-3.5 h-3.5 {{ $dateString === $twoDaysAgoString ? 'text-white' : 'text-amber-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    ...
                    <button 
                        ...
                        class="px-2.5 py-1.5 bg-tealBrand hover:bg-teal-700 text-white text-[11px] font-bold rounded-lg shadow-2xs transition cursor-pointer"
                    >
                        Hari Ini
                    </button>
                    @endif
                ...
                <div class="flex items-center gap-1.5 flex-wrap">
                    <button 
                        ...
                        class="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition flex items-center gap-1.5 shadow-2xs cursor-pointer"
                    ...
                    <a href="{{ route('kitchen-reports.download-template', ['branch_id' => $activeBranch->id]) }}" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition flex items-center gap-1.5 border border-slate-200" title="Unduh template Excel">
                        <svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                        <span>Template</span>
                    </a>
                    <a href="{{ route('daily-expenses.index') }}" class="px-3 py-2 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 text-xs font-bold rounded-xl transition flex items-center gap-1.5">
                        <svg class="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                        </svg>
                        <span>Belanja</span>
                    </a>
                    <a href="{{ route('kitchen-reports.index') }}" class="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded-xl transition">
                        Kembali
                    </a>
                </div>
            </div>
        ...
        <!-- Form Parameter Selector (Cabang, Tanggal Kustom, Sisa Kemarin) -->
        <div class="pt-3 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
            <!-- 1. Cabang -->
            <div class="relative" id="createBranchDropdownWrapper">
                @if(auth()->user()->isSuperAdmin() || (auth()->user()->isKepalaCabang() && count($branches) > 1))
                    ...
                    <button 
                        ...
                        class="w-full flex items-center justify-between px-3 py-2 text-xs bg-slate-50 hover:bg-white border border-slate-200 rounded-xl text-slate-800 font-bold hover:border-tealBrand focus:outline-none transition-colors cursor-pointer shadow-2xs"
                    >
                        <div class="flex items-center space-x-2 truncate">
                            <span class="w-2 h-2 rounded-full bg-emerald-500 shrink-0"></span>
                            <span class="text-slate-400 font-medium">Cabang:</span>
                ...
                @else
                    <input type="hidden" name="branch_id" id="createBranchInput" value="{{ $activeBranch->id }}">
                    <div class="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 flex items-center space-x-2">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 shrink-0"></span>
                        <span class="text-slate-400 font-medium">Cabang:</span>
            ...
            <div class="relative">
                ...
                <input 
                    ...
                    id="reportDateInput" 
                    value="{{ $dateString }}" 
                    class="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 focus:bg-white border border-slate-200 rounded-xl text-slate-800 font-bold focus:outline-none focus:border-tealBrand cursor-pointer shadow-2xs transition"
                    title="Klik untuk memilih tanggal laporan"
                >
            </div>

            <!-- 3. Status Sisa Kemarin -->
            <div class="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs flex items-center justify-between">
                <div class="flex items-center space-x-1.5 truncate">
                    <svg class="w-3.5 h-3.5 {{ $previousReport ? 'text-tealBrand' : 'text-slate-400' }} shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span class="text-slate-500 font-medium">Sisa Kemarin:</span>
                    <span class="font-bold text-slate-800 truncate">
                        {{ $previousReport ? $previousReport->report_date->translatedFormat('d M Y') : 'Diset 0' }}
                    </span>
                </div>
                @if($previousReport)
                    <span class="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-emerald-100 text-emerald-800 shrink-0">Tersambung</span>
                @else
                    <span class="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-slate-200 text-slate-600 shrink-0">Awal (0)</span>
                @endif
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('kitchen-reports.store') }}" method="POST" id="kitchenReportForm" class="space-y-6">
        ...
        <input type="hidden" name="report_date" value="{{ $dateString }}">
                ...
                <div class="overflow-x-auto max-h-[600px]">
                    <table class="w-full text-left border-collapse text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-800 font-medium" id="dishesTableBody">
                            ...
                            <tr class="hover:bg-slate-50/60 transition item-row" data-index="{{ $index }}" data-menu-id="{{ $item['menu_id'] }}" data-name="{{ strtolower($item['menu_name']) }}" data-perishable="{{ $item['is_perishable'] ? '1' : '0' }}">
